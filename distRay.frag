...
// Constants for Distribution Ray Tracing
const int SPP = 32; // Number of samples (random primary rays, random secondary rays) per pixel.
                      ...
                      out float t, out vec3 hitPos, out vec3 hitNormal )
{
    ...
    vec3 rayOrigin = ray.o - sph.center; 
    
    float a = dot(ray.d, ray.d); // should be 1 since rayDirection is unit vector
    float b = 2.0 * dot(ray.d, rayOrigin);
    float c = dot(rayOrigin, rayOrigin) - pow(sph.radius, 2.0); 
    float d = pow(b, 2.0) - (4.0 * a * c); // discriminant
    
    if ( d < 0.0 ) { 
        // no intersection since sqrt(d) where d < 0 will result in non-real results
        return false; 
    } else {
        float tplus = (-b + sqrt(d)) / (2.0 * a); 
        float tminus = (-b - sqrt(d)) / (2.0 * a);
        
        if (tminus >= tmin && tminus <= tmax) {
            t = tminus;
        } else if (tplus >= tmin && tplus <= tmax) {
            t = tplus;
        } else {
            // both tMin and tPlus do not fall between tMin and tMax
            return false;
        }
                      ...
                      in float tmin, in float tmax )
{
    ...
    if ( d < 0.0 ) { 
        ...
        return (tminus >= tmin && tminus <= tmax) || (tplus >= tmin && tplus <= tmax);
    }
}
              ...
              out vec3 hitNormal, out vec3 k_rg )
{
    ...
    // Intersect with all planes and update nearest intersection
    for (int planeIdx = 0; planeIdx < NUM_PLANES; planeIdx++) {
        temp_hasHit = IntersectPlane(Plane[planeIdx], ray, DEFAULT_TMIN, DEFAULT_TMAX, temp_t, temp_hitPos, temp_hitNormal);
        if (temp_hasHit && temp_t < nearest_t) {
            hasHitSomething = true;
            nearest_t = temp_t; 
            nearest_hitPos = temp_hitPos; 
            nearest_hitNormal = temp_hitNormal; 
            nearest_hitMatID = Plane[planeIdx].materialID; 
        }
    }

    // Intersect with all spheres and update nearest intersection
    for (int sphereIdx = 0; sphereIdx < NUM_SPHERES; sphereIdx++) { 
        temp_hasHit = IntersectSphere(Sphere[sphereIdx], ray, DEFAULT_TMIN, DEFAULT_TMAX, temp_t, temp_hitPos, temp_hitNormal);
        if (temp_hasHit && temp_t < nearest_t) { 
            hasHitSomething = true;
            nearest_t = temp_t; 
            nearest_hitPos = temp_hitPos; 
    ...
    // * Implement soft shadows as a bonus.
    ///////////////////////////////////////////////////////////////////////////
    ...
    for (int lightIdx = 0; lightIdx < NUM_LIGHTS; lightIdx++) {
        temp_I_local = vec3(0.0);
        shadowRay.o = nearest_hitPos;
        
        // Find vectors orthogonal to light, and to one another
        vec3 w = normalize(Light[lightIdx].position - nearest_hitPos);
        vec3 u = vec3(0.0, 1.0, -w.y / w.z); // Satisfies condition dot(u, w) = 0
        u = normalize(u);
        vec3 v = normalize(cross(u, w));
        
        for (int sampleIdx = 0; sampleIdx < SPP; sampleIdx++) {
        
            shadowRay.d = normalize(Light[lightIdx].position + ((rand() - 0.5) * u) + ((rand() - 0.5) * v) - nearest_hitPos);
            ...
            while (!temp_hasHit && planeIdx < NUM_PLANES) {
                temp_hasHit = IntersectPlane(Plane[planeIdx], shadowRay, DEFAULT_TMIN, distance(shadowRay.o, Light[lightIdx].position));
                planeIdx++;
            }

            // Check for intersection with spheres
            while (!temp_hasHit && sphereIdx < NUM_SPHERES) {
                temp_hasHit = IntersectSphere(Sphere[sphereIdx], shadowRay, DEFAULT_TMIN, distance(shadowRay.o, Light[lightIdx].position));
                sphereIdx++;
            }

            temp_I_local += PhongLighting(shadowRay.d, nearest_hitNormal, normalize(ray.o - shadowRay.o), temp_hasHit, Material[nearest_hitMatID], Light[lightIdx]);
        }
        I_local += temp_I_local / float(SPP);
    }
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for (int sphereIdx = 0; sphereIdx < SPP; sphereIdx++) {
    
        // Get random position on lens.
        // Lens is centered at origin of camera frame (cam_pos) and parallel to x-y plane of camera frame (cam_x_axis, cam_y_axis). 
        temp_cam_pos = cam_pos + aperture_width * ((rand() - 0.5) * cam_x_axis + (rand() - 0.5) * cam_y_axis);
        
        // Get random position inside current pixel.
        temp_pixel_pos = image_origin + pixel_width * vec3(fragCoord + vec2(rand() - 0.5, rand() - 0.5), 0.0);
       
        // Create primary ray.
        pRay.o = temp_cam_pos;
        pRay.d = normalize( cam_pos + temp_pixel_pos.x * cam_x_axis  +
                                      temp_pixel_pos.y * cam_y_axis  +
                                      temp_pixel_pos.z * cam_z_axis - pRay.o);
        ...
        totalFragColor += vec4( I_result, 1.0 );
    }
    
    fragColor = totalFragColor / float(SPP);
}
